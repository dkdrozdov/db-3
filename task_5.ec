...
/*      Задание 5. 
 *      Выполнить запрос:
 *          Выдать полную информацию об изделиях, у которых есть
 *          поставки ТОЛЬКО с весом от 1000 до 7000.    
 */
void query(){
    ...
    exec SQL declare cursor1 cursor for
        select distinct j.*
        from j
        where j.n_izd in(select spj1.n_izd
                            from spj spj1
                            join p p1 on p1.n_det=spj1.n_det
                            where spj1.kol*p1.ves>=1000 and spj1.kol*p1.ves<=7000
                            except (select spj1.n_izd
                                    from spj spj1
                                    join p p1 on p1.n_det=spj1.n_det
                                    where spj1.kol*p1.ves<1000 or spj1.kol*p1.ves>7000));
    ...
    while(sqlca.sqlcode != 100) // Проверка на достижение конца выборки.
    {
        ...
        if(!data_read) printf("| %-10s | %-21s | %-21s |\n", "n_izd", "name", "town");
        data_read = true;
